...
import type { BiModistasResumen, Granularidad } from "@/lib/domain/analytics/types";
...
type ModistasChartProps = {
  resumen: BiModistasResumen;
  granularidad: Granularidad;
};

export function ModistasChart({ resumen, granularidad }: ModistasChartProps) {
    ...
    <div className="rounded-xl border border-pastel-border bg-pastel-surface p-4">
      ...
      {chartData.length === 0 ? (
        <div className="flex h-48 items-center justify-center text-sm text-pastel-text/50">
          Sin costos de modista en el período
        </div>
      ) : (
