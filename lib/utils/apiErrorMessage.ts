...
export function getUserFacingErrorMessage(error: unknown): string {
  ...
  if (error.status >= 500) {
    ...
    message =
      "La accion entro en conflicto con otro cambio reciente. Refresca e intenta de nuevo.";
  } else if (error.status === 401) {
    message = "No estas autenticado. Vuelve a iniciar sesion.";
  } else if (error.status === 403) {
    message =
      "Tu sesion no tiene permisos para esta accion. Vuelve a iniciar sesion.";
